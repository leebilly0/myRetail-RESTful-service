...
public class DemoApplication {
	...
	public Product findByid(@PathVariable String id) {
		Product externalProduct = getExternalAPIData(id); // Get External Product
	...
	public String updateByID(@PathVariable String id, @RequestBody Product in_product) {
		...
		if (in_product.getId() != id) { // If IDs don't match, kick back response
			try {
				jsonString.put("responseCode", "409");
			...
			try {
				jsonString.put("responseCode", "406");
		...
		try {
			jsonString.put("responseCode", "200");
